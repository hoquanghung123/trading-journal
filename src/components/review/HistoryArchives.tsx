...
export function HistoryArchives({
  reviews,
  selectedPeriod,
  onSelectPeriod,
  onDelete,
}: HistoryArchivesProps) {
          ...
          <div className="overflow-y-auto max-h-[500px] divide-y divide-border/50">
            {filteredReviews.length === 0 ? (
              ...
              filteredReviews.map((rev) => {
                ...
                const actionCount =
                  rev.actionPlan.hardRules.length +
                  rev.actionPlan.optimization.length +
                  rev.actionPlan.training.length;
                  ...
                  <HoverCard key={rev.id} openDelay={200}>
                    <HoverCardTrigger asChild>
                      ...
                      >
                        <div className="col-span-8 flex items-center gap-4">
                          ...
                          {isSelected && (
                            <div className="px-2 py-0.5 rounded-md bg-primary/20 text-[10px] font-black uppercase ml-2">
                              Active
                            </div>
                          )}
                        </div>
                              ...
                              <AlertDialogContent className="bg-white border-border rounded-2xl shadow-2xl">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription className="text-muted-foreground font-medium">
                                    ...
                                    <span className="font-bold text-primary">{rev.period}</span>.
                    ...
                    >
                      <div className="bg-primary/5 px-6 py-4 border-b border-border/50 flex items-center justify-between">
                        <div>
                          <h4 className="text-primary font-black text-sm uppercase tracking-wider">
                            {rev.period} Snapshot
                          </h4>
                        ...
                        <Badge className="bg-primary text-white border-none text-[9px] px-3 py-1 rounded-lg shadow-sm">
                          {rev.type}
                        </Badge>
                      </div>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {rev.technicalReflection && (
                              <div className="bg-muted/30 p-3 rounded-xl border border-border/50">
                                <span className="text-[9px] font-black uppercase text-primary/60 block mb-1">
                                  Technical
                                </span>
                            ...
                            {rev.psychologicalReflection && (
                              <div className="bg-muted/30 p-3 rounded-xl border border-border/50">
                                <span className="text-[9px] font-black uppercase text-purple-600/60 block mb-1">
                                  Psychological
                                </span>
                                <p className="text-xs text-foreground font-medium line-clamp-2 leading-relaxed">
                                  {rev.psychologicalReflection}
                            ...
                            {rev.environmentalReflection && (
                              <div className="bg-muted/30 p-3 rounded-xl border border-border/50">
                                <span className="text-[9px] font-black uppercase text-amber-600/60 block mb-1">
                                  Environmental
                                </span>
                                <p className="text-xs text-foreground font-medium line-clamp-2 leading-relaxed">
                                  {rev.environmentalReflection}
                            ...
                            <div className="space-y-2">
                              {rev.topMistakes
                                ...
                                .map((m, i) => (
                                  <div
                                    key={i}
                                    className="flex items-start gap-3 bg-destructive/[0.03] p-2.5 rounded-xl border border-destructive/10 group"
                                  >
                                    <span className="text-destructive/40 text-[10px] font-black mt-0.5">
                                      #0{i + 1}
                                    </span>
                                    <span className="text-xs text-foreground font-bold leading-tight line-clamp-2">
                                      {m.text}
                                    </span>
                                  </div>
                                ))}
