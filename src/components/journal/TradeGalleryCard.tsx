...
export function TradeGalleryCard({ trade, onClick, showGrade }: TradeGalleryCardProps) {
  const [hovered, setHovered] = useState(false);
  const outcome = computeOutcome(trade.actualRr);
    ...
    >
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-muted">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg ${
            trade.side === "buy" ? "bg-primary text-primary-foreground" : "bg-destructive text-destructive-foreground"
          }`}>
            {trade.side}
          </span>
          {showGrade && trade.grade && (
            <span className={`px-2.5 py-0.5 rounded-lg text-[9px] font-black shadow-lg ${gradeStyle[trade.grade] || ""}`}>
              {trade.grade}
            </span>
          )}
        </div>
      ...
      <div className="p-5 flex flex-col flex-1 gap-4">
        <div className="flex items-center justify-between">
          ...
          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm ${style}`}>
            {outcome.label}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 rounded-2xl bg-muted/30 border border-border/30">
            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Profit/Loss</p>
            <div className="flex items-center gap-1.5">
              ...
              <span className={`text-sm font-black ${trade.netPnl >= 0 ? "text-primary" : "text-destructive"}`}>
                {trade.netPnl >= 0 ? "+" : ""}{trade.netPnl.toFixed(2)}
              </span>
            </div>
          </div>
          <div className="p-3 rounded-2xl bg-muted/30 border border-border/30">
            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1">RR Achieved</p>
            <p className="text-sm font-black text-foreground">
              {trade.actualRr} <span className="text-[10px] text-muted-foreground font-medium">/ {trade.maxRr}</span>
            </p>
          </div>
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-border/30 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[9px] font-bold uppercase tracking-widest ${trade.complianceCheck ? "text-primary" : "text-destructive"}`}>
