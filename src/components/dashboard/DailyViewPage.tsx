...
import { fetchTrades, deleteTrade, upsertTrade, type Trade, computeOutcome, outcomeStyle } from "@/lib/trades";
      ...
      <div className="max-w-[1100px] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="bg-white rounded-3xl border border-border p-4 pr-6 flex items-center gap-5 shadow-sm">
             ...
             <div>
                <div className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-0.5">Total PnL</div>
                <div className={`text-xl font-black ${totalPnl >= 0 ? "text-primary" : "text-destructive"}`}>
        ...
        <div className="bg-white rounded-[40px] border border-border shadow-sm overflow-hidden min-h-[500px] flex flex-col">
          {loading ? (
            <div className="flex-1 flex flex-col items-center justify-center p-20">
              ...
              <p className="mt-4 text-sm font-bold text-muted-foreground uppercase tracking-widest">Fetching executions...</p>
            </div>
          ) : trades.length === 0 ? (
            ...
            <div className="divide-y divide-border/50">
              {trades.sort((a, b) => +new Date(a.entryTime) - +new Date(b.entryTime)).map((t, i) => {
                  ...
                  >
                    <div className="flex items-center gap-8">
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                          <span className="text-xl font-black text-foreground tracking-tight group-hover:text-primary transition-colors">{t.symbol}</span>
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                            t.side === "buy" ? "bg-primary/10 text-primary border border-primary/20" : "bg-destructive/10 text-destructive border border-destructive/20"
                          }`}>
                            {t.side}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm ${outcomeStyle[outcome.color]}`}>
                            {outcome.label}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="text-right flex items-center gap-10">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Risk/Reward</span>
                        <span className="text-sm font-bold text-foreground">
                          {t.actualRr} <span className="text-muted-foreground/30 mx-1">/</span> {t.maxRr}
                        </span>
                      </div>

                      {/* Profit */}
                      <div className="flex flex-col items-end gap-0.5 min-w-[120px]">
                        <span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Net Result</span>
                        <span className={`text-2xl font-black ${
                          t.netPnl > 0 ? "text-primary" : t.netPnl < 0 ? "text-destructive" : "text-muted-foreground"
                        }`}>
