    ...
    <div className="p-6 bg-orange-50 dark:bg-orange-950/20 rounded-[32px] border border-orange-100 dark:border-orange-900/30 shadow-sm h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-bold text-orange-900 dark:text-orange-400 uppercase tracking-widest">
            Top Mistakes
          </h3>
        </div>
        {period && (
        ...
        {validMistakes.length > 0 ? (
          validMistakes.map((m, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-start gap-3 p-3 bg-white/50 dark:bg-orange-900/10 rounded-2xl border border-orange-200/50 dark:border-orange-800/30"
              >
                <input
                  type="checkbox"
                ...
                />
                <span
                  className={`text-xs font-medium ${isFixed ? "line-through text-orange-400" : "text-orange-800 dark:text-orange-300"}`}
                >
                  {text}
                </span>
              </div>
          ...
          <p className="text-xs text-orange-600/50 italic p-4 text-center">
            No mistakes logged in last review.
          </p>
        )}
      </div>
    </div>
      ...
      <div className="space-y-6">
        {categories.map((cat, i) => (
          <div key={i} className="space-y-2">
            <h4 className={`text-[10px] font-black uppercase tracking-[0.2em] ${cat.color}`}>
              {cat.label}
            </h4>
            <div className="space-y-2">
              {cat.items.filter(
                (item) => (typeof item === "string" ? item : item.text).trim() !== "",
              ).length > 0 ? (
                cat.items
                  ...
                  .map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-2 group cursor-pointer">
                        <div
                          className={`w-1 h-1 rounded-full ${isChecked ? "bg-primary" : "bg-slate-300 dark:bg-slate-700"} group-hover:bg-primary transition-colors`}
                        />
                        <span
                          className={`text-xs transition-colors ${isChecked ? "text-slate-400 line-through" : "text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200"}`}
                        >
                          {text}
                        </span>
                      </div>
