  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (env.SUPABASE_PUBLISHABLE_KEY)
        globalThis.SUPABASE_PUBLISHABLE_KEY = env.SUPABASE_PUBLISHABLE_KEY;
      ...
      if (url.pathname.startsWith("/storage/")) {
        ...
        try {
          ...
          return new Response("Not Found", {
            status: 404,
            headers: {
              "X-Debug-All": JSON.stringify({
                ...debug,
                source: "NOT-FOUND",
                sb_status: response.status,
              }),
            },
          });
        } catch (error) {
          return new Response(error.message, {
            status: 500,
            headers: {
              "X-Debug-All": JSON.stringify({ ...debug, source: "ERROR", error: error.message }),
            },
          });
        }
      }
      ...
      if (isPageRequest) {
        ...
        const isJson =
          request.headers.get("accept")?.includes("json") || url.searchParams.has("_data");
