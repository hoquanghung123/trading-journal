...
function getSymbol() {
  return document.querySelector('[data-name="legend-item-symbol-name"]')?.innerText || 
         document.querySelector('.title-W_9_V67a')?.innerText ||
         document.title.split(' ')[0].split(':').pop() || "UNKNOWN";
}
...
async function syncToJournal() {
  ...
  const cameraBtn = document.querySelector('[data-name="save-screenshot"]') || 
  ...
  let oldClip = "";
  try { oldClip = await navigator.clipboard.readText(); } catch(e) {}
  ...
  setTimeout(() => {
    ...
    const menuInterval = setInterval(async () => {
      ...
      const copyLinkBtn = Array.from(allElements).find(el => {
        ...
        return (txt.includes('copy') || txt.includes('sao chép')) && (txt.includes('link') || txt.includes('liên kết'));
      });

      if (copyLinkBtn) {
        clearInterval(menuInterval);
        const clickable = copyLinkBtn.closest('li') || copyLinkBtn.closest('[class*="item"]') || copyLinkBtn;
        clickable.click();
        ...
        const clipInterval = setInterval(async () => {
          try {
            ...
            if (text.includes('tradingview.com/x/') && text !== oldClip && text !== lastSyncedUrl) {
              ...
              setTimeout(() => { createButton(mainBtn); }, 2000);
            }
          } catch (err) {}
          clipAttempts++;
          if (clipAttempts > 60) { clearInterval(clipInterval); createButton(mainBtn); }
        }, 300);
      }
      attempts++;
      if (attempts > 50) { clearInterval(menuInterval); createButton(mainBtn); }
    }, 50);
  }, 300);
}
...
function createButton(container) {
  ...
  b.innerHTML = '<span style="writing-mode: vertical-rl; text-orientation: mixed; letter-spacing: 1px;">SYNC</span>';
  b.style.cssText = "background:#8CC63F; color:white; border:none; border-radius:3px; padding:6px 2px; cursor:pointer; font-weight:bold; font-size:9px; width:22px; height:65px; transition:all 0.2s; display:flex; align-items:center; justify-content:center; box-shadow: 0 2px 8px rgba(0,0,0,0.4); text-transform: uppercase; opacity: 0.9;";
  b.onmouseover = () => { b.style.background = "#7ab337"; b.style.opacity = "1"; };
  b.onmouseout = () => { b.style.background = "#8CC63F"; b.style.opacity = "0.9"; };
  b.onclick = (e) => {
    e.stopPropagation();
    syncToJournal();
  };
...
function injectButton() {
  ...
  parent.style.cssText = "position: fixed; right: 0; top: 120px; display: flex; flex-direction: column; align-items: center; background: rgba(0, 0, 0, 0.7); padding: 4px 2px; border-radius: 6px 0 0 6px; z-index: 1000000; border: 1px solid rgba(140, 198, 63, 0.3); border-right: none; backdrop-filter: blur(5px); transition: opacity 0.3s;";
  createButton(parent);
